...
async function main() {
    ...
    const scenarioArg = args.find(a => a.startsWith('--scenario='))?.split('=')[1] || 'average-age';
    ...
    switch (scenarioArg) {
        case 'hypertension':
            ...
            payload = NmpCompiler.compileAnalysis(SCENARIO_HYPERTENSION, "HypertensionAnalytics");
            break;
        case 'average-age':
            console.log(`🔧 [Compiler] Compilando Transformación Estadística (Anonimizada)...`);
            payload = NmpCompiler.compileAnalysis(SCENARIO_AVERAGE_AGE, "AgeAnalytics");
            break;
        case 'ast-attack':
            console.log(`⚠️  [Compiler] Empaquetando Lógica Maliciosa (Sandbox Escape Attempt)...`);
            console.log(`⚠️  [Compiler] ADVERTENCIA: Este ataque debería ser bloqueado en Cero-Tiempo por el Guardian AST.`);
        ...
        case 'fuel-exhaustion':
            ...
            console.log(`⚠️  [Compiler] ADVERTENCIA: El WASI Sandbox debería detener esto amputando el Fuel.`);
            payload = NmpCompiler.compileAnalysis(SCENARIO_FUEL_EXHAUSTION, "LogicBomb");
            break;
        default:
            console.error(`❌ Escenario desconocido. Use: hypertension, average-age, ast-attack, fuel-exhaustion`);
            process.exit(1);
    }
    ...
    try {
        console.log(`🚀 [Injector] Enviando Payload Logic-on-Origin al Nodo Remoto...`);
        ...
        if (result.isError) {
            ...
            if (msg && 'text' in msg) {
                ...
                console.log(`✅ [ZK Verifier] Prueba Criptográfica Válida: ${data.zk_receipt}`);
            }
        }
