...
async function main() {
    ...
    const payload = NmpCompiler.compileAnalysis(SCENARIO_DIABETES, "DiabetesAnalytics");
    ...
    if (result.isError) {
        if (msg && typeof msg.text === "string") console.error(`🛡️ Bloqueado:`, msg.text);
    } else {
        if (msg && typeof msg.text === "string") {
            const data = JSON.parse(msg.text);
            console.log(`\n💎 RESULTADO EXACTO DE LA CONSULTA:`, data.computation_result);
