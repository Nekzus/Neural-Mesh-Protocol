...
export class GuardianAST {
    private static FORBIDDEN_PATTERNS = [
        "fetch", "http", "process", "require", "eval", "os", "fs.readSync", "XMLHttpRequest"
    ];
    ...
    static validate(payload: Buffer): boolean {
        ...
        for (const pattern of this.FORBIDDEN_PATTERNS) {
            if (content.includes(pattern)) {
                console.error(`[GuardianAST] ❌ CRITICAL: Security Violation! Found forbidden pattern: "${pattern}"`);
                return false;
            }
        }

        console.log(`[GuardianAST] ✅ Pre-Check Passed. No sandbox escapes detected.`);
        return true;
    }
}
