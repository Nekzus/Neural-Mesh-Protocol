...
export class WasiSandbox {
     ...
     */
    static async execute(payload: Buffer, context: any): Promise<{ result: any; receipt: any }> {
        ...
        for (const record of records) {
            ...
            if (consumedFuel > this.MAX_FUEL) {
                throw new Error("[WasiSandbox] TRAP: Fuel Exhaustion (Infinite Loop detected or Budget exceeded)");
            }
        ...
        console.log(`[WasiSandbox] Execution Success. Fuel Consumed: ${consumedFuel} units.`);
        ...
        const seal = createHash("md5").update(JSON.stringify(journal)).digest("hex");
